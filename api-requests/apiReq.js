...
export const getCommentsByBrand = async (brandId) => {
  try {
    const response = await axios.get(
      `${process.env.NEXT_PUBLIC_API_URL}/comments?brandId=${brandId}/comments`
    );
    return response.data;
  } catch (error) {
    return [];
...
export const getAllBlogsAndEvents = async (
  page = 1,
  pageSize = 6,
  prefixLink
) => {
  try {
    const response = await axios.get(
      `${process.env.NEXT_PUBLIC_API_URL}/${prefixLink}`,
      {
        params: {
          page,
          pageSize,
...
export const postFeedback = async (data) => {
  try {
    const response = await axios.post(
      `${process.env.NEXT_PUBLIC_API_URL}/contact-me`,
      data
    );
    return response.data;
  } catch (error) {
...
export const getWebSocialMedias = async () => {
  try {
    const response = await axios.get(
      `${process.env.NEXT_PUBLIC_API_URL}/website-socials`
    );
    return response.data;
  } catch (error) {
    return [];
